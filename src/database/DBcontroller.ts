...
export class DbController {
    ...
    getOneById = async <T>(column: keyof T, value: unknown, tableName: string) => {
        ...
        if (!Boolean(data.length)) throw new DbNotFoundError("No entries found", 404)
        return data[0] as T;
    }
    getEntries = async <T>(column: keyof T, value: unknown, tableName: string, options: QueryOptions = {}) => {
        const columnName = String(column);
